...
df['hobbie_list'] = df['essay_combined'].apply(hobbies_find)

# how many hobbies for each person?
df['hobbie_count'] = df['hobbie_list'].apply(num_hobby)
...
df.to_csv('text_analysis.csv', index=False)
...
plt.title('k value and Silo score')
plt.show()

# Try k = 5 in kmeans 
# k_pca = KMeans(5)
# pca_labs = k_pca.fit_predict(pca_df)

# skplot.metrics.plot_silhouette(pca_df, pca_labs, title="KMeans-5-PCA", figsize=(15,5))
# plt.show()

# df['pca_label'] = pca_labs
...
### ====================================TSNE+KMeans with specific groups==================================== ###
...
col = ['diet_halal', 'diet_kosher', 'diet_veggie', 'body_type_above average',
    ...
    #    'serious', 'importance', 
       ...
       'ent_cardinal','ent_nord','ent_person','ent_time','ent_org',
       'ent_ordinal','ent_gpe','hobbie_count',
       'hobbie_list'
]
...
d = MinMaxScaler().fit_transform(tmp_f.T)
ds_tmp = pd.DataFrame(d,columns=tmp_f.T.columns).T
sns.heatmap(ds_tmp, cmap='Purples', fmt='g',cbar=False)
plt.title('Female Descriptive Statistics')
plt.show()
...
d = MinMaxScaler().fit_transform(tmp_m.T)
ds_tmp = pd.DataFrame(d,columns=tmp_m.T.columns).T
sns.heatmap(ds_tmp, cmap='Purples', fmt='g',cbar=False)
plt.title('Male Descriptive Statistics')
plt.show()
...
all_text_str = np.concatenate(df.hobbie_list)

from wordcloud import WordCloud
all_text_str = ' '.join(all_text_str)
wc = WordCloud(background_color="white")
wordcloud = wc.generate(all_text_str)
plt.imshow(wordcloud)
plt.axis("off")
plt.show()


df.to_csv('text_analysis_2.csv', index=False)